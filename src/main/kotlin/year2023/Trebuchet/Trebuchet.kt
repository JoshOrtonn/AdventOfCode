...
class Trebuchet {
    ...
    fun decipherDocument(sampleInput: List<String>? = null): Int {
        ...
        input.forEach {
            ...
            val firstChar = charArray.first { char -> char.isDigit() }
            val lastChar = charArray.last { char -> char.isDigit() }


            val concatenatedValue: Int = "$firstChar$lastChar".toInt()

            runningTotal += concatenatedValue
        }

        println("Running total was: $runningTotal")
        return runningTotal
    }
}
